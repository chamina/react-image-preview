...
const e: React.FC<{ imageData: any, imageDeselect: (val: any) => any }> = function ImageDisplay({ imageData , imageDeselect}) {
  const images = imageData.map((image: string) => {
    return <span className="close" onClick ={() => imageDeselect(image)}> x<img style={style} width={500} height={300} src={image} /></span>;
